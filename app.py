...
def get_firebase_data():
    ...
    return {
        "tank_level": 75,
        "planters": [{"moisture": 50, "lower": 30, "upper": 70} for _ in range(5)]
    }
...
if not st.session_state.logged_in:
    ...
    firebase_planters = sys_data.get("planters", [{"moisture": 50, "lower": 30, "upper": 70} for _ in range(5)])

    # חיווי מפלס מים והתרעה אם המיכל ריק
    if tank_level <= 10:
        st.error(
    ...
    def fetch_ai_irrigation_data(plant_name):
        ...
        prompt = f"""
        You are a strict agricultural botanical expert.
        Check if the input text "{plant_name}" is a real, valid plant, flower, vegetable, herb, or tree.
        If it is complete gibberish, a fake name, or not a plant at all, respond strictly with: {{"valid": false}}
        If it is a valid plant, provide exact irrigation thresholds and respond with JSON:
        {{"valid": true, "lower": <min_moisture_percentage>, "upper": <max_moisture_percentage>}}
        """
        try:
            ...
            return {"valid": True, "lower": 35, "upper": 68}
    ...
    for i in range(5):
        with cols[i]:
            ...
            if plant != st.session_state.previous_plants[i] and plant != "":
                with st.spinner("מאמת צמח ומנתח ספים..."):
                    ai_data = fetch_ai_irrigation_data(plant)
                    if ai_data and ai_data.get("valid", True):
                        st.session_state.ai_thresholds[i] = {"lower": ai_data['lower'], "upper": ai_data['upper']}
                        update_thresholds_in_firebase(i, ai_data['lower'], ai_data['upper'])
                    else:
                        st.error(f"❌ הצמח '{plant}' אינו מוכר או שאינו קיים. נא להזין שופ צמח אמיתי.")
                        st.session_state.ai_thresholds[i] = {"lower": None, "upper": None}
            ...
            try:
                ...
                stored_lower = firebase_planters[i].get("lower", 35)
                stored_upper = firebase_planters[i].get("upper", 70)
            except:
                current_moisture = 50
                stored_lower = 35
                stored_upper = 70
