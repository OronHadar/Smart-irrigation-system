...
if not st.session_state.logged_in:
    ...
    def fetch_ai_irrigation_data(plant_name):
        ...
        Provide exact, unique, non-generic moisture thresholds for this specific plant based on botanical science.
        ...
        try:
            ...
            elif "עגבני" in name or "מלפפון" in name:
                return {"lower": 52, "upper": 85}
            elif "רוזמרין" in name or "טימין" in name:
                return {"lower": 24, "upper": 58}
            elif "גזר" in name:
                return {"lower": 32, "upper": 68}
            else:
    ...
    for i in range(5):
        with cols[i]:
            ...
            # בדיקה האם השם השתנה והוזן צמח חדש
            if plant != st.session_state.previous_plants[i] and plant != "":
                with st.spinner("מנתח ספי צימאון מדויקים עם AI..."):
            ...
            # שליפת הנתונים מהזיכרון המקומי או מה-AI
            ...
            # הצגת הספים המעודכנים (מה-AI או מהענן)
