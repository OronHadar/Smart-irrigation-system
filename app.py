...
# רענון אוטומטי של הדף כל 5 שניות כדי לראות נתונים חמים מהבקר
...
def get_firebase_data():
    try:
        ...
        pass
    return {"tank_level": 50, "planters": [{"moisture": 50, "lower": 30, "upper": 70} for _ in range(5)]}
...
if not st.session_state.logged_in:
    ...
    st.markdown(
        f"<p class='sub-title'>מרכז בקרה וטלמטריה חי (מתרענן אוטומטית) | מנוהל על ידי {st.session_state.profile_name}</p>",
        unsafe_allow_html=True)

    firebase_data = get_firebase_data()
    tank_level = firebase_data.get("tank_level", 50)
    planters_data = firebase_data.get("planters", [{"moisture": 50, "lower": 30, "upper": 70} for _ in range(5)])
    ...
    def get_plant_icon(plant_name):
        ...
        if "בצל" in name or "שום" in name: return "🧅"
        if "עגבני" in name: return "🍅"
        if "גזר" in name: return "🥕"
        if "מלפפון" in name or "קישוא" in name: return "🥒"
        if "חציל" in name: return "🍆"
        if "פלפל" in name or "חריף" in name: return "🌶️"
        if "בזיליקום" in name or "ריחן" in name or "רוזמרין" in name: return "🌿"
        if "נענע" in name or "מנטה" in name: return "🍃"
        if "תות" in name: return "🍓"
        if "אבטיח" in name: return "🍉"
        if "תפוז" in name or "לימון" in name: return "🍊"
        if "קקטוס" in name or "סוקולנט" in name: return "🌵"
        if "פרח" in name or "ורד" in name: return "🌹"
        return "🪴"
    ...
    def fetch_ai_irrigation_data(plant_name):
        ...
        prompt = f"""
        Provide exact optimal soil moisture thresholds for the plant: "{plant_name}".
        Return ONLY valid JSON format with integer percentages:
        {{"lower": <integer_between_20_and_55>, "upper": <integer_between_65_and_90>}}
        """
        try:
            ...
            res = json.loads(raw_text[start_idx:end_idx + 1])
    ...
    for i in range(5):
        with cols[i]:
            ...
            if plant != st.session_state.previous_plants[i] and plant != "":
                with st.spinner("מנתח ספי צימאון ומשדר לבקר..."):
            ...
            try:
                ...
                low_th = p_data.get("lower", 35)
                up_th = p_data.get("upper", 70)
                current_moisture = p_data.get("moisture", 50)
            except:
                low_th, up_th, current_moisture = 35, 70, 50

            if plant:
                st.markdown(f"""
                    <div class='ai-threshold-box'>
                        💧 סף הפעלה (צימאון): {low_th}%<br>
                        🛑 סף עצירה (אידיאלי): {up_th}%
                    </div>
                """, unsafe_allow_html=True)
            else:
            ...
            bar_color = "#3498db"
            if current_moisture <= low_th:
                bar_color = "#e74c3c"
            elif current_moisture >= up_th:
                bar_color = "#9b59b6"
            else:
                bar_color = "#2ecc71"
