...
if not st.session_state.logged_in:
    ...
    def fetch_ai_irrigation_data(plant_name):
        if not st.session_state.api_key: return {"error": "API key is missing"}
        genai.configure(api_key=st.session_state.api_key)

        try:
            # מנגנון גילוי אוטומטי - שואל את גוגל אילו מודלים זמינים ולוקח את הראשון שעובד!
            ...
            if not working_model_name:
                return {"error": "המפתח שלך לא מורשה לאף מודל שמייצר טקסט. בדוק את ההרשאות ב-Google Cloud."}
        ...
        with cols[i]:
            ...
            if plant != st.session_state.previous_plants[i]:
                if plant == "":
                    ...
                    with st.spinner("מוצא מודל מתאים ומנתח..."):
                        ai_data = fetch_ai_irrigation_data(plant)

                        if ai_data and "error" in ai_data:
                            st.error(f"❌ שגיאת מערכת: {ai_data['error']}")
                            st.session_state.planter_thresholds[i] = {"lower": None, "upper": None}
