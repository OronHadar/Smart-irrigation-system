...
def get_firebase_data():
    try:
        ...
        pass
    return {"tank_level": 0, "planters": [{"moisture": 0} for _ in range(5)]}
...
if not st.session_state.logged_in:
    ...
    # שליפת נתונים חיים מהענן
    firebase_data = get_firebase_data()
    tank_level = firebase_data.get("tank_level", 0)
    planters_data = firebase_data.get("planters", [{"moisture": 0} for _ in range(5)])
    ...
    def fetch_ai_irrigation_data(plant_name):
        ...
        prompt = f"""
        You are a strict agricultural expert. The user planted: "{plant_name}".
        Provide exact, unique moisture thresholds. Return ONLY valid JSON:
        {{"lower": <min_moisture>, "upper": <max_moisture>}}
        """
        try:
    ...
    for i in range(5):
        with cols[i]:
            ...
            # אם הצמח השתנה - נמשוך מה-AI ונדחוף מיד לענן!
            if plant != st.session_state.previous_plants[i] and plant != "":
                with st.spinner("מנתח ספי צימאון מדויקים ומשדר לבקר..."):
            ...
            # משיכת הלחות האמיתית מה-Firebase (ולא מספר הדמה)
            try:
                current_moisture = planters_data[i].get("moisture", 0)
            except IndexError:
                current_moisture = 0
            ...
            bar_color = "#3498db"
            if thresholds['lower']:
                if current_moisture <= thresholds['lower']:
                    bar_color = "#e74c3c"
                elif current_moisture >= thresholds['upper']:
                    bar_color = "#9b59b6"
                else:
                    bar_color = "#2ecc71"
