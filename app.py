...
# אתחול המשתנים ששומרים את הנתונים האמיתיים כדי למנוע ריפרש כפול
if 'previous_plants' not in st.session_state:
    st.session_state.previous_plants = ["", "", "", "", ""]
if 'planter_thresholds' not in st.session_state:
    st.session_state.planter_thresholds = [{"lower": None, "upper": None} for _ in range(5)]
...
def get_firebase_data():
    ...
    return {
        "tank_level": 75,
        "planters": [{"moisture": 50} for _ in range(5)]
    }
...
if not st.session_state.logged_in:
    ...
    sys_data = get_firebase_data()
    tank_level = sys_data.get("tank_level", 75)
    firebase_planters = sys_data.get("planters", [{"moisture": 50} for _ in range(5)])

    if tank_level <= 10:
        st.error(
            f"🚨 אזהרה קריטית: מפלס המים במיכל עומד על {tank_level}% בלבד! המיכל כמעט ריק לחלוטין, יש למלא מים מיד!")
    else:
        st.info(f"💧 מפלס המים הנוכחי במיכל: {tank_level}%")
    ...
    # פונקציה נקייה - מתחברת ל-AI בלבד ואין לה שום גיבוי מספרי ידני!
    def fetch_ai_irrigation_data(plant_name):
        if not st.session_state.api_key: return None
        genai.configure(api_key=st.session_state.api_key)
        model = genai.GenerativeModel('gemini-1.5-flash')
        prompt = f"""
        You are a strict botanical expert.
        Check if "{plant_name}" is a real plant/vegetable/herb/tree in Hebrew or English.
        If fake/gibberish, return EXACTLY: {{"valid": false}}
        If valid, return the exact optimal soil moisture thresholds (in percentage) for this specific plant.
        Return ONLY valid JSON format:
        {{"valid": true, "lower": <integer>, "upper": <integer>}}
        """
        try:
            ...
            if res.get("valid", True):
                return {"valid": True, "lower": int(res["lower"]), "upper": int(res["upper"])}
            else:
                return {"valid": False}
        except Exception as e:
            return None  # אם יש שגיאה, מחזיר שגיאה ולא מספרים סתם.
    ...
    for i in range(5):
        with cols[i]:
            ...
            # בדיקה האם השם של הצמח הוחלף עכשיו. רק אז קוראים ל-AI!
            if plant != st.session_state.previous_plants[i]:
                if plant == "":
                    st.session_state.planter_thresholds[i] = {"lower": None, "upper": None}
                    st.session_state.previous_plants[i] = ""
                else:
                    with st.spinner("ה-AI מנתח ספים בוטניים אמיתיים..."):
                        ai_data = fetch_ai_irrigation_data(plant)
                        if ai_data is None:
                            st.error(f"❌ שגיאה בחיבור ל-AI. נסה להקליד שוב.")
                            st.session_state.planter_thresholds[i] = {"lower": None, "upper": None}
                        elif ai_data.get("valid", True):
                            st.session_state.planter_thresholds[i] = {
                                "lower": ai_data['lower'],
                                "upper": ai_data['upper']
                            }
                        else:
                            st.error(f"❌ '{plant}' אינו צמח מוכר.")
                            st.session_state.planter_thresholds[i] = {"lower": None, "upper": None}
            ...
            try:
                current_moisture = firebase_planters[i].get("moisture", 50)
            except:
                current_moisture = 50

            thresholds = st.session_state.planter_thresholds[i]

            # מציג ספים רק אם ה-AI החזיר נתונים אמיתיים
            if plant and thresholds['lower'] is not None:
                display_lower = thresholds['lower']
                display_upper = thresholds['upper']
                st.markdown(f"""
