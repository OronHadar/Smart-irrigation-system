...
if not st.session_state.logged_in:
    ...
    if 'ai_thresholds' not in st.session_state:
        st.session_state.ai_thresholds = [{"lower": 35, "upper": 70} for _ in range(5)]
    ...
    def fetch_ai_irrigation_data(plant_name):
        ...
        Check if the input text "{plant_name}" is a real, valid plant, flower, vegetable, herb, or tree in Hebrew or English.
        If it is complete gibberish, a fake name, or not a plant at all, respond strictly with: {{"valid": false}}
        If it is a valid plant, provide exact, unique botanical irrigation thresholds and respond with JSON:
        {{"valid": true, "lower": <min_moisture_percentage_integer>, "upper": <max_moisture_percentage_integer>}}
        """
        try:
            ...
            return {"valid": True, "lower": 40, "upper": 75}
    ...
    for i in range(5):
        with cols[i]:
            ...
            if plant != st.session_state.previous_plants[i] and plant != "":
                with st.spinner("מאמת צמח ומנתח ספים ייחודיים..."):
                    ai_data = fetch_ai_irrigation_data(plant)
                    if ai_data and ai_data.get("valid", True):
                        st.session_state.ai_thresholds[i] = {
                            "lower": ai_data['lower'],
                            "upper": ai_data['upper']
                        }
                    else:
                        st.error(f"❌ הצמח '{plant}' אינו מוכר. נא להזין שם אמיתי.")
